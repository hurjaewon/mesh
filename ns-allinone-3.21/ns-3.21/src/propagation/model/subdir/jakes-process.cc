...
//shbyeon snr decrease within ppdutxtime
double*
JakesProcess::GetChannelGainDb (Time time) const
{
  std::complex<double> complexGain = std::complex<double> (0, 0);
  std::complex<double> complexGain_ref = std::complex<double> (0, 0);
  Time sumNow = Now() + time; 
  for (unsigned int i = 0; i < m_oscillators.size (); i++)
    {
      complexGain += m_oscillators[i].GetValueAt (sumNow);
      complexGain_ref += m_oscillators[i].GetValueAt (Now());
    }

  double amp = 10 * std::log10 ((std::pow (complexGain_ref.real (), 2) + std::pow (complexGain_ref.imag (), 2)) / 2);
  double a = complexGain_ref.real();
  double b = complexGain_ref.imag();
  double c = complexGain.real();
  double d = complexGain.imag();
  double squareValue = (a*a + b*b);
  double newValue = std::sqrt((squareValue-a*c-b*d)*(squareValue-a*c-b*d)+(a*d-b*c)*(a*d-b*c));
  //newValue = newValue / squareValue;
  double final = newValue/squareValue;
/*
  if(1 > newValue)
    final = 1 - newValue;
  else
