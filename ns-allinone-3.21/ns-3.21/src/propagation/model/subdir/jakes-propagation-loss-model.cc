                                          ...
                                          double interval) const
{
  ...
  if (pathData == 0)
    ...
    Time refTime = Seconds ((double)interval);
    evmToSNR = pathData->GetChannelGainDb (refTime);
    evmToSNR[0] += txPowerDbm;
    NS_LOG_DEBUG("rxPower=" << evmToSNR[0] << " " << evmToSNR[1]);
    if(interval == 0)
      return evmToSNR[0];
    else
      return evmToSNR[1];
                                          ...
                                          Ptr<MobilityModel> b) const
{
  ...
  NS_LOG_DEBUG(txPowerDbm  << " " << pathData->GetChannelGainDb ());
