...
MacLow::ReceiveOk (Ptr<Packet> packet, double rxSnr, WifiMode txMode, WifiPreamble preamble)
{
  ...
  bool isPrevNavZero = IsNavZero ();
	NS_LOG_DEBUG ("recv " << hdr.GetTypeString () <<
                ", from=" << hdr.GetAddr2 () <<
                ", size=" << packet->GetSize () <<
...
MacLow::GetBlockAckDuration (Mac48Address to, WifiTxVector blockAckReqTxVector, enum BlockAckType type) const
{
  ...
  if (blockAckReqTxVector.GetMode().GetModulationClass () == WIFI_MOD_CLASS_VHT)
    ...
    preamble=WIFI_PREAMBLE_LONG;
  NS_LOG_DEBUG ("BlockAckMode=" << blockAckReqTxVector.GetMode ()<<"\tduration="<<m_phy->CalculateTxDuration (GetBlockAckSize (type), blockAckReqTxVector, preamble));
                     ...
                     WifiTxVector txVector, WifiPreamble preamble)
{
  ...
  ErrorFreeTag tag;
  //if (hdr->IsMgt ())
  if (hdr->IsMgt () || hdr->IsBlockAck() || hdr->IsAck())
	  tag.Set (true);
  else
	  tag.Set (false);
...
MacLow::AggregateMpdu(Ptr<MpduAggregator> aggregator)
{
  ...
  //shbyeon find optimal rate
  if(m_stationManager->m_optLength)
  {
    NS_LOG_DEBUG("find optimal length here");
    m_stationManager->FindOptLength(m_currentHdr.GetAddr1(), &m_currentHdr, dataTxMode, (int)dataTxVector.GetNss());
  }
