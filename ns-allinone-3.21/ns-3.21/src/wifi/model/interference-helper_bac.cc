...
InterferenceHelper::CalculateAmpduSnrPer (Ptr<InterferenceHelper::Event> event, uint32_t size, uint16_t ampduSize, Ptr<Packet> packet)
{
  ...
  for(k=0;k<ampduSize+1;k++)
    per[k]=0;
  CalculateAmpduPer(event, &ni, packet, per);
  struct SnrPer snrPer;
  for(k=0;k<ampduSize+1;k++)
  {
    snrPer.snr = snr;
    ...
    NS_LOG_DEBUG("subframe=" << k << "th' per=" << per[k]);
  }
  return snrPers;
}
//shbyeon ampdu per calculation
void
InterferenceHelper::CalculateAmpduPer (Ptr<const InterferenceHelper::Event> event, NiChanges *ni, Ptr<Packet> packet, double per[]) const
{
	NS_LOG_FUNCTION(this);
  NiChanges::iterator j = ni->begin ();
  Time previous = (*j).GetTime ();
  ...
  while(ni->end () != j)
  {
    ...
    if(previous >= plcpPayloadStart)
    {
      ...
      {
        psr[subframeIndex] *= CalculateChunkSuccessRate (CalculateSnr (powerW, noiseInterferenceW, payloadMode),
...
InterferenceHelper::Determinant (std::complex<double> **mtx, int size)
 {
   ...
   //NS_LOG_UNCOND("determinant: "<<det);
   //NS_LOG_UNCOND("mtx: "<<mtx[0][0]<<"  "<<mtx[0][1]<<"  "<<mtx[1][0]<<"  "<<mtx[1][1]);
   return(det);
 }
void 
