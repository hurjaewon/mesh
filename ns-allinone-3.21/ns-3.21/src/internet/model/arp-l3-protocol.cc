...
namespace ns3 {
    ...
    Ptr<ArpCache>
        ArpL3Protocol::FindCache (Ptr<NetDevice> device)
        {
            ...
            /*            //JWHUR accomodate bridge network device
                          for (CacheList::const_iterator i = m_cacheList.begin (); i != m_cacheList.end (); i++)
                          {
                          if ((*i)->GetDevice ()->IsBridge())
                          {
                          return *i;
                          }
                          }*/
    ...
    void 
                ...
                const Address &to, NetDevice::PacketType packetType)
        {
            ...
            for (uint32_t i = 0; i < cache->GetInterface ()->GetNAddresses (); i++)
            {
                ...
                {
                    Mac48Address arpDest = Mac48Address::ConvertFrom(arp.GetDestinationHardwareAddress ());
                    if (device->IsBridge ())
                    {
                        if (DynamicCast<BridgeNetDevice> (device)->FindAddress (arpDest))
                        {
                            found = true;
                    ...
                    else if (arp.GetDestinationHardwareAddress () == device->GetAddress ())
                    {
                        found = true;
                        Ipv4Address from = arp.GetSourceIpv4Address ();
                        ArpCache::Entry *entry = cache->Lookup (from);
    ...
    void
        ArpL3Protocol::SendArpRequest (Ptr<const ArpCache> cache, Ipv4Address to)
        {
            ...
            //JWHUR accomodate bridge case 
            if (device->IsBridge ())
                DynamicCast<BridgeNetDevice> (device)->SendArp (packet, device->GetBroadcast (), PROT_NUMBER);
            else
                device->Send (packet, device->GetBroadcast (), PROT_NUMBER);
        }
    ...
    void
        ArpL3Protocol::SendArpReply (Ptr<const ArpCache> cache, Ipv4Address myIp, Ipv4Address toIp, Address toMac)
        {
            ...
            //JWHUR accomodate bridge case 
            if (device->IsBridge ())
                DynamicCast<BridgeNetDevice> (device)->SendArp (packet, toMac, PROT_NUMBER);
            else
                device->Send (packet, toMac, PROT_NUMBER);
