...
main (int argc, char *argv[])
{
  ...
  double staGain = 0;
	...
	double map1_x = 1, map1_y = 1;
	double map2_x = 1, map2_y = 1;
	int map1_pos = 10, map2_pos = 9;
	double apTxpower = 16.0206;
	double staTxpower = 16.0206;
  bool m_pcap = true;
  ...
  cmd.Parse (argc, argv);
	...
	switch(map1_pos)
	{
		...
		case 3:
			...
			break;
		case 4:
			map1_x = 3.5; map1_y = 13.5;
			break;
		case 5:
			map1_x = 13; map1_y = 13;
			break;
		case 6:
			map1_x = 8; map1_y = 10.5;
			break;
		case 7:
			map1_x = 13; map1_y = 10.5;
			break;
		case 8:
			map1_x = 18; map1_y = 10.5;
			break;
		case 9:
			map1_x = 3; map1_y = 8;
			break;
...
switch(map2_pos)
	{
		...
		case 3:
			...
			break;
		case 4:
			map2_x = 3.5; map2_y = 13.5;
			break;
		case 5:
			map2_x = 13; map2_y = 13;
			break;
		case 6:
			map2_x = 8; map2_y = 10.5;
			break;
		case 7:
			map2_x = 13; map2_y = 10.5;
			break;
		case 8:
			map2_x = 18; map2_y = 10.5;
			break;
		case 9:
			map2_x = 3; map2_y = 8;
			break;
	...
	double sta_pos[nsta][3] = {9.0, 21.0, 1.0, 19.0, 20.0, 1.0, 8.0, 14.0, 1.0, 7.0, 7.0, 1.0, 14.0, 7.0, 1.0, 10.0, 4.0, 1.0, 4.0, 1.0, 1.0};
  ...
  staInterfaces2 = apAddress2.Assign (staDevices2);
	...
	Ptr<Ipv4> ipv4mesh3 = meshNodes.Get(2)->GetObject<Ipv4> ();

	Ipv4StaticRoutingHelper ipv4RoutingHelper;
	Ptr<Ipv4StaticRouting> staticRoutingMesh1 = ipv4RoutingHelper.GetStaticRouting (ipv4mesh1);
	Ptr<Ipv4StaticRouting> staticRoutingMesh2 = ipv4RoutingHelper.GetStaticRouting (ipv4mesh2);
	staticRoutingMesh1->AddNetworkRouteTo (Ipv4Address ("192.168.2.0"), Ipv4Mask("255.255.255.0"), Ipv4Address("10.1.1.2"), 1);
	staticRoutingMesh2->AddNetworkRouteTo (Ipv4Address ("192.168.3.0"), Ipv4Mask("255.255.255.0"), Ipv4Address("10.1.1.3"), 1);
	...
	for (int i = 0; i < (int)staNodes1.GetN(); i++)
	{
		PacketSinkHelper sink ("ns3::UdpSocketFactory", InetSocketAddress (staInterfaces1.GetAddress (i), 9));
		apps_sink.Add (sink.Install (staNodes1.Get(i)));
	}
	for (int i = 0; i < (int)staNodes2.GetN(); i++)
	{
		PacketSinkHelper sink ("ns3::UdpSocketFactory", InetSocketAddress (staInterfaces2.GetAddress (i), 9));
		apps_sink.Add (sink.Install (staNodes2.Get(i)));
	}
	for (int i = 0; i < (int)staNodes3.GetN(); i++)
	{
		PacketSinkHelper sink ("ns3::UdpSocketFactory", InetSocketAddress (staInterfaces3.GetAddress (i), 9));
		apps_sink.Add (sink.Install (staNodes3.Get(i)));
	}

	
	for (int i = 0; i < (int)staNodes1.GetN(); i++)
	{
		OnOffHelper onoff ("ns3::UdpSocketFactory", InetSocketAddress (staInterfaces1.GetAddress (i), 9));;
		onoff.SetConstantRate (DataRate ("10Mb/s"), m_packetSize);
	...
	for (int i = 0; i < (int)staNodes2.GetN(); i++)
	{
		OnOffHelper onoff ("ns3::UdpSocketFactory", InetSocketAddress (staInterfaces2.GetAddress (i), 9));;
		onoff.SetConstantRate (DataRate ("10Mb/s"), m_packetSize);
  	onoff.SetAttribute ("OnTime", StringValue ("ns3::ConstantRandomVariable[Constant=1000]"));
		onoff.SetAttribute ("StartTime", TimeValue (Seconds (1.0)));
  	onoff.SetAttribute ("StopTime", TimeValue (Seconds (m_totalTime)));
	...
	for (int i = 0; i < (int)staNodes3.GetN(); i++)
	{
		OnOffHelper onoff ("ns3::UdpSocketFactory", InetSocketAddress (staInterfaces3.GetAddress (i), 9));;
		onoff.SetConstantRate (DataRate ("10Mb/s"), m_packetSize);
  	onoff.SetAttribute ("OnTime", StringValue ("ns3::ConstantRandomVariable[Constant=1000]"));
		onoff.SetAttribute ("StartTime", TimeValue (Seconds (1.0)));
  	onoff.SetAttribute ("StopTime", TimeValue (Seconds (m_totalTime)));
  ...
  apps_sink.Stop (Seconds (m_totalTime + 1));
	...
	std::cout << "( " << map1_pos << ", " << map2_pos << " )\tRSSI:";
  ...
  std::cout << "\tdBm,Throughput:\t";
	for (int i = 0; i < nsta; i++)
		std::cout << throughput[i] << "\t";
	std::cout << sumThp << "\tMbps\n";
