...
main(int argc, char *argv[])
{
  ...
  channel.AddPropagationLoss("ns3::Winner2PropagationLossModel", "Frequency", DoubleValue(5.15));
  ...
  staMob->SetVelocity (Vector (1.0, 0.0, 0.0));
  ...
  for (uint32_t user = 0; user < users; user++)
    {
      DashClientHelper client("ns3::TcpSocketFactory",InetSocketAddress(ap_interfaces.GetAddress(0), port),protocol);
