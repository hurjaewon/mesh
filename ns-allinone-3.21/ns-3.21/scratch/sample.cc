...
void 
PrintRecordResult(char* fn, int dist, double thpt, double aggr, double sfer, int iter, int ampdu, int lowRate, 
...
int inst_idx=0;
static void
CaudalLoss_ (Mac48Address addr, uint16_t totalTx, uint16_t Nsuc, double aggrTime, int mcs, int antenna, uint16_t bw, bool lowerRate)
{
...
int main (int argc, char *argv[])
{
  ...
  //LogComponentEnable ("ns3::ArfWifiManager", LOG_LEVEL_INFO);
	...
	cmd.AddValue ("Bound", "aggregation bound", bound);
	cmd.Parse (argc,argv);

  NS_LOG_UNCOND("Iter:" << iter << " Dist:" << dist << " Bandwidth:" << bandwidth << " SimulationTime:" << sim_time 
      << "\nMoFA:" << MoFA << " eMoFA:" << eMoFA << " rateCtrl:" << rateCtrl << " Caudal:" << caudal << " Opt:" << opt_length << " Dvel:" << dopplerVel);
  ...
  if(MoFA)
	  Config::SetDefault("ns3::WifiRemoteStationManager::MoFA", BooleanValue(true));
  else if(eMoFA)
	  Config::SetDefault("ns3::WifiRemoteStationManager::eMoFA", BooleanValue(true));
  else if(opt_length)
	  Config::SetDefault("ns3::WifiRemoteStationManager::OptLength", BooleanValue(true));
  ...
  phy.Set("Receivers", UintegerValue(nss));
	  ...
	  if(vht)	
    {
      ...
      mac.SetMaxPpduTime (AC_BE, MilliSeconds (bound));
    }
    else
    {
	...
	for (int k=0;k<nsta;k++){
		...
		switch (k){
			case 0:
				positionAllocSta[k]->Add (Vector (dist, 0.0, 0.0));
				Stamobility.SetPositionAllocator (positionAllocSta[k]);
				Stamobility.Install (wifiStaNodes.Get(0));
				break;
			case 1:
				positionAllocSta[k]->Add (Vector (65.0, 0.0, 0.0));
				Stamobility.SetPositionAllocator (positionAllocSta[k]);
				Stamobility.Install (wifiStaNodes.Get(1));
				break;
			case 2:
				positionAllocSta[k]->Add (Vector (85.0, 0.0, 0.0));
				Stamobility.SetPositionAllocator (positionAllocSta[k]);
				Stamobility.Install (wifiStaNodes.Get(2));
				break;
			case 3:
				positionAllocSta[k]->Add (Vector (5.5, 1.0, 0.0));
				Stamobility.SetPositionAllocator (positionAllocSta[k]);
				Stamobility.Install (wifiStaNodes.Get(3));
				break;
			case 4:
				positionAllocSta[k]->Add (Vector (11.0, 4.0, 0.0));
				Stamobility.SetPositionAllocator (positionAllocSta[k]);
				Stamobility.Install (wifiStaNodes.Get(4));
				break;
			default:
				positionAllocSta[k]->Add (Vector (k, 0.0, 0.0));
				Stamobility.SetPositionAllocator (positionAllocSta[k]);
  ...
  NS_LOG_UNCOND ("Nss\t"<<nss<<"\tDist\t"<<dist<<"\tMCS\t"<<mcs<<"\tTotalTput\t" << tot_thruput 
          << "\tSFER\t" <<  sfer << "\tAGGR\t" << aggrTime << "\tTOTAL\t" << tx_tot 
          << "\tSUCCESS\t" << tx_suc << "\tLOWERRATE\t" << tx_lowerRate << "\tAMPDU\t" << tx_ampdu);

  double total_inst = 0;
  for(int i=0; i<10; i++)
  {
//    NS_LOG_UNCOND("INSTANT THROUGHPUT\t" << i << "\t" << inst_thpt[i]);
    total_inst+=inst_thpt[i];
  }
  NS_LOG_UNCOND("# OF MPDU " << total_inst);

  char fn[255];
  sprintf(fn,"/home/2011/shbyeon2/80211acSim/results/150715/VERSION_%d/Dist_%d-Caudal_%d-MoFA_%d-eMoFA_%d-Rc_%d-Dvel_%.1f-Iter_%d.txt",version,(int)dist,caudal,MoFA,eMoFA,rateCtrl,dopplerVel,iter);
  PrintRecordResult (fn,dist,tot_thruput,aggrTime,sfer,iter,tx_ampdu,tx_lowerRate,
