...
//
  ...
  cmd.AddValue("targetDt","The target time difference between receiving and playing a frame.",target_dt);
  ...
  cmd.AddValue("protocol","The adaptation protocol. It can be 'ns3::DashClient' or 'ns3::OsmpClient (for now).",protocol);
...
//
  ...
  NetDeviceContainer de_sta1;
  NetDeviceContainer de_ap1; 
  ...
  wifi1.SetRemoteStationManager("ns3::ArfWifiManager");
  ...
  mac1.SetType("ns3::StaWifiMac","Ssid", SsidValue(ssid1),"ActiveProbing", BooleanValue(true));
...
//    wifiPhy.Set ("ChannelNumber", UintegerValue(1));
  ...
  staMob->SetPosition (Vector (0.0, 3.0, 0.0));
  staMob->SetVelocity (Vector (0.5, 0.0, 0.0));
  */
...
//
  ...
  Ipv4InterfaceContainer i = ipv4.Assign(de_wifi_sta1Ap1);
