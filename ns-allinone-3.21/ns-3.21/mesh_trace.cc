...
main (int argc, char *argv[])
{
  ...
  bool mac_ampdu = true;
  uint32_t seed = 10;
	int nsta = 7, n1 = 3, n2 = 2, n3 = 4;

	int dr[n1 + n2 + n3] = {4, 8, 7, 10, 10, 6, 8, 4, 10};
	int set1[n1] = {0, 3, 5};
	int set2[n2] = {3, 4};
	int set3[n3] = {1, 2, 3, 6};
	std::string dataStr;
  ...
  cmd.Parse (argc, argv);
	...
	double sta_pos[nsta][3] = {9.0, 21.0, 1.0, 19.0, 20.0, 1.0, 8.0, 14.0, 1.0, 14.0, 7.0, 1.0, 7.0, 7.0, 1.0, 10.0, 4.0, 1.0, 4.0, 1.0, 1.0};
  ...
  staInterfaces2 = apAddress2.Assign (staDevices2);
	...
	for (int i = 0; i < n1; i++)
	{
		ipv4sta = staNodes.Get(set1[i])->GetObject<Ipv4> ();
		Ipv4Address addr = ipv4sta->GetAddress (1, 0).GetLocal();
		...
		onoff.SetAttribute ("StartTime", TimeValue (Seconds (1.0)));
 		onoff.SetAttribute ("StopTime", TimeValue (Seconds (31.0)));
		apps_source.Add (onoff.Install (meshNodes.Get(0)));
	}

	for (int i = 0; i < n2; i++)
	{
		ipv4sta = staNodes.Get(set2[i])->GetObject<Ipv4> ();
		Ipv4Address addr = ipv4sta->GetAddress (1, 0).GetLocal();
		OnOffHelper onoff ("ns3::TcpSocketFactory", InetSocketAddress (addr, 9));
		std::ostringstream datarate;
		datarate << dr[n1 + i];
		...
		onoff.SetConstantRate (DataRate (dataStr), m_packetSize);
 		onoff.SetAttribute ("OnTime", StringValue ("ns3::ConstantRandomVariable[Constant=1000]"));
		onoff.SetAttribute ("StartTime", TimeValue (Seconds (31.0)));
 		onoff.SetAttribute ("StopTime", TimeValue (Seconds (61)));
		apps_source.Add (onoff.Install (meshNodes.Get(0)));
	}

	for (int i = 0; i < n3; i++)
	{
		ipv4sta = staNodes.Get(set3[i])->GetObject<Ipv4> ();
		Ipv4Address addr = ipv4sta->GetAddress (1, 0).GetLocal();
		OnOffHelper onoff ("ns3::TcpSocketFactory", InetSocketAddress (addr, 9));
		std::ostringstream datarate;
		datarate << dr[n1 + n2 + i];
		...
		onoff.SetConstantRate (DataRate (dataStr), m_packetSize);
 		onoff.SetAttribute ("OnTime", StringValue ("ns3::ConstantRandomVariable[Constant=1000]"));
		onoff.SetAttribute ("StartTime", TimeValue (Seconds (61.0)));
