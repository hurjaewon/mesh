...
main (int argc, char *argv[])
{
  ...
  double staGain = 0;
	...
	double outlet[14][2] =	{
		...
		11, 5,
		9, 1};
	int map1_pos = 9;//, map2_pos = 9;
	double ap5Txpower = 17;
	double ap2_4Txpower = 5;
	double staTxpower = 17;
  bool m_pcap = false;
...
//  cmd.AddValue ("ap_txpower", "Tx power of ap stations (dB)", apTxpower);
  ...
  NetDeviceContainer apDevice1, apDevice2, brDevice1, brDevice2, staDevices1, staDevices2;
  //NetDeviceContainer apDevice1, apDevice2, apDevice3, brDevice1, brDevice2, brDevice3, staDevices1, staDevices2, staDevices3;
  ...
  wifiPhy.Set ("TxGain", DoubleValue (apGain), "RxGain", DoubleValue (apGain), "TxPowerStart", DoubleValue (ap5Txpower), "TxPowerEnd", DoubleValue (ap5Txpower));
  ...
  wifiPhy.Set ("TxGain", DoubleValue (apGain), "RxGain", DoubleValue (apGain), "TxPowerStart", DoubleValue (ap2_4Txpower), "TxPowerEnd", DoubleValue (ap2_4Txpower), "ChannelNumber", UintegerValue(40));
...
*/
  ...
  apps_sink.Stop (Seconds (m_totalTime + 1));
	...
	Simulator::Schedule (Seconds (m_totalTime), Report);
